...
module.exports = class PetController {
    ...
    static async getPetById(req,res){
        ...
        if(!pet){
            res.status(404).json({message:'Pet não encontrado inválido '})
            return
        }
        res.status(200).json({
            pet:pet
    ...
    static async removePetById(req,res){
        ...
        console.log(pet.user._id + 'pet')
        console.log(user._id + 'user')
        if(pet.user._id.toString() !== user._id.toString()) {
            res.status(422).json({message:'Houve um problema na solicitação,tente novamente mais tarde '})
        }
    ...
    static async updatePet(req,res){
        ...
        if(!name){
            res.status(422).json({message:'O name é obrigatorio'})
            return
        }else{
    ...
    static async schedule(req,res){
        ...
        if(pet.user._id.equals(user._id)){
            res.status(422).json({message: 'Vocẽ não pode agendar uma visita para seu própio Pet !'})
            return
        } 
    ...
    static async concludeAdoption(req,res){
        ...
        //check id logged in user registered the pet
        const token = getToken(req)
        const user = await getUserByToken(token)
        
        if(pet.user._id.toString() !== user._id.toString()){
            res.status(422).json({
                message:'Houve um problema em processar sua solicitação,tente mais tarde novamente!'
            })
            return
        }
        ...
        res.status(201).json({ message : 'Parabéns o ciclo de adoção foi concluido com sucesso!'})
